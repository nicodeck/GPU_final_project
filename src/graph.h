...
private:
	...
	{
		arc_st			*first = nullptr;		/* first outcoming arc */

		arc_st			*parent = nullptr;	/* node's parent */
		node_st			*next = nullptr;		/* pointer to the next active node
									   (or to itself if it is the last node in the list) */
		int				TS = 0;			/* timestamp showing when DIST was computed */
		int				DIST = 0;		/* distance to the terminal */
		short			is_sink = 0;	/* flag showing whether the node is in the source or in the sink tree */
	...
	{
		node_st			*head = nullptr;		/* node the arc points to */
		arc_st			*next = nullptr;		/* next arc with the same originating node */
		arc_st			*sister = nullptr;	/* reverse arc */
	...
	{
		node_st			*ptr = nullptr;
		nodeptr_st		*next = nullptr;
	} nodeptr;

	Block<node>			*node_block = nullptr;
	Block<arc>			*arc_block = nullptr;
	DBlock<nodeptr>		*nodeptr_block = nullptr;
...
/***********************************************************************/
	...
	inline void process_source_orphan(node *i);
	inline void process_sink_orphan(node *i);
};
